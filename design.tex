...
\section{Cyclic Graphs in Rust}\label{sec:graph}
  ...
  \subsection{Native Cycles}
    ...
    Secondly has \codr{y} to be assigned to \codr{x.next} and with that needs \codr{x} to be mutable.
  ...
  \subsection{Memory Arenas and Cells}\label{sec:memoryArena}
    \autoref{sec:cell} introduced \UnsafeCellT and companion types were introduced.
  ...
  \subsection{Indices as Pointers Replacement}
    ...
    While an \gls{ecs} or the optimisation in \citetitle{skill-scala}\autocite{skill-scala} seems favorable to the previous solutions it was ruled out by Timm Felden.
...
\section{Inheritance in Rust}\label{sec:inheritance}
  ...
  \subsection{Inheritance through Composition}%
    ...
    As alternative an option form \autoref{sec:graph} could be used e.g. \codr{Option<Rc<RefCell<T>>>} to create inheritance cycles.
...
\section{Support Cyclic Graphs and Struct Inheritance}
  ...
  \subsection{Memory Layout}
    ...
    The \codr{PhantomData<T>} to provide the compiler with information which \enquote{is used when computing certain safety properties.}\autocite[std::marker::PhantomData]{rust-doc}.
  ...
  \subsection{Specification Independent Code}
    ...
    \subsubsection{Foreign Types and Fields}
      ...
      This is possible as the user has no way to interact with the foreign types or fields in the Rust implementation\footnote{%
        In \citetitle{skill-tr13}\autocite{skill-tr13} the \gls{api} to access foreign types and fields was a feature, in \citetitle{skill-tr}\autocite{skill-tr} it was changed to be a core requirement. For this thesis Timm Felden said to treat it as feature.
      }.
