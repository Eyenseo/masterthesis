...
\section{Graphs}\label{sec:graph}
  ...
  \subsection{Native Cycles}
    ...
    \autoref{lst:cyclesTwo} uses the \codr{Option} wrapper introduced in \autoref{ssec:rustErr}.
  ...
  \subsection{Smart Pointer}
    ...
    If the strong counter reaches zero in the destructor (called \codr{Drop} in Rust) of \RcT the destructor for \codr{T} is called, but the memory is not released yet.
    ...
    It is possible to have any number of \RcTs and \WeakTs to the same \codr{T}.
    If there is more than one \RcT or a \WeakT to the same \codr{T} it is only possible to access \codr{T} in an immutable way.
    ...
    The solution to this issue is to use an additional wrapper \codr{Rc<RefCell<T>>}.
    \codr{RefCell<T>} uses internally \codr{UnsafeCell<T>} and provides the means to enforce the borrowing rules and thread-safety guarantees.
    ...
    When calling \codr{borrow()} or \codr{borrow_mut()} on \codr{RefCell<T>} it will not return a \codr{&T} or \codr{&mut T}, instead \codr{Ref<T>} and \codr{RefMut<T>} are returned.
...
\section{Inheritance}
  ...
  \subsection{Traits and Black Magic}\label{ssec:magic}
    ...
    \newsavebox{\inheritanceTriatOne}
    \begin{lrbox}{\inheritanceTriatOne}
    ...
    \newsavebox{\inheritanceTriatTwo}
    \begin{lrbox}{\inheritanceTriatTwo}
    ...
    \begin{figure}[ht]
      \captionsetup{type=lstlisting}
      \usebox{\inheritanceTriatOne}\hfill%
      \usebox{\inheritanceTriatTwo}
    ...
    In \autoref{ssec:stg} trait objects were introduced as fat pointer and this property can be abused to implement the \codr{cast} function.
...
\section{Generated Code}
  ...
  \subsection{VTable-Lookup-Table}
    ...
    % TODO It is important to understand that with the restrictions Rust enforces on generic functions every cast will create a trait object -- even if the cast actually is supposed to create a \struct.
